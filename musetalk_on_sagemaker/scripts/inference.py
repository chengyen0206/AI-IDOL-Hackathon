...
def main(args):
    ...
    for task_id in inference_config:
        try:
            ...
            # ##################################################
            # example_frame = cv2.imread(f"{result_img_save_path}/00000000.png")
            # height, width = example_frame.shape[:2]
            # if height % 2 != 0 or width % 2 != 0:
            #     print(f"Warning: Adjusting frame size from ({width}, {height}) to even numbers.")
            #     new_width = width if width % 2 == 0 else width - 1
            #     new_height = height if height % 2 == 0 else height - 1
            #     for img_file in glob.glob(f"{result_img_save_path}/*.png"):
